	...
	for ( i = 0; i < width; i++ )
	{
		for ( j = 0; j < height; j++ )
		{
			...
			if ( i * j % 2 ) 
			{
				n->x = i;
				n->y = j;
                n->dirs = 15; //Assume that all directions can be explored (4 youngest bits set) bianry
				n->c = ' '; 
			}
...
void saveMaze(){
    ...
    // create unperfect maze by using arguement percent
    int temp[width*height];
    for(i=0; i<width*height; ++i) temp[i] = i;
    for(i=width*height-1;i>0;--i) swap(temp[i], temp[rand()%i]);
    for(x=0; x<width*height*percent/100; x++){
        int u = temp[x];
        maze.dig(u%width, (u-u%width)/width);
    }
    maze.save();
...
int main( int argc, char **argv )
{
    ...
    if ( sscanf( argv[1], "%d", &width ) + sscanf( argv[2], "%d", &height )+ sscanf( argv[3], "%d", &percent )< 3 )
	{
    ...
    {
        fprintf( stderr, "%s: The percent should in 0~100!\n", argv[0] );
        exit( 1 );
    }
